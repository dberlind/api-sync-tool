...
The are various tools for designing RAML-based APIs. You could create an account on the Anypoint platform and use the Web-based API Designer which is built into that platform's API Manager section:
...
Next, assuming you have an API design to sync and an account on the Anypoint platform, it's time to run API-Sync. Before running API-Sync though, you should change directories to the same directory that where the root RAML file of the design you want to sync to Anypoint is stored. Where the option exists to supply the name of the root RAML file as a parameter, that parameter can only be the filename (in other words, no path). So, it's important for your working directory to be the same one as where the root RAML file is kept or API-Sync will not work.
...
5. The API's version number (ie: 1, 1.1, 2, etc). Where version number is required as a parameter, trailing zeroes are truncated. For example, "1.0" will be turned into "1". Like with API Name (see point 2), this parameter is only needed when you're using API-Sync to add a new API to Anypoint.

### Finding the Anypoint Business Group ID
As of the publication of this document, there is no menu-driven way to report on the Business Group ID. But the easiest way to find it is to inspect the URL while your browser looking at an existing API or the API Manager home page where all your APIs are listed. The Business Group ID is the full set of number and digits that appears after the part of the URL that says "/organizations/" (highlighted in the screenshot below):
...
![Screenshot of Anypoint API Version URL](ReadMe_Images/business-group-id.jpg "Finding an Anypoint API ID")
...
## Available options
Listed below are the main options that are available to the API-Sync utility. Each option, with examples, explained in more detail below the list.
...
- Then, you will presented with a list of APIs to work with and after that, a list of specific versions. Please note that when working with the interactive version, you're essentially setting these parameters so that they continue to apply through multiple runs of API-Sync. To clear all the setting and start fresh (for example, if you're going to work with a different API, run "api-sync cleanup")
- Once you've indicated what existing API you're going to work with, you'll be asked if you want to pull your API definition now. Answering yes will download the selected API design and all supporting files to the current working directory. You would probably do this in a case where you're going to manipulate the design with a local tool like API Designer or Atom (running the API Workbench plug-in). Just remember that once you change the design, you might want to sync the new design to Anypoint under a new version number. If you answer no to this question, then API-Sync will remember what you've established for the different parameters so that you don't have to supply that information on subsequent runs.

![Screenshot of API-Sync Setup](ReadMe_Images/api-sync-setup-screenshot.jpg "Screenshot of API-Sync Setup")

Setup can also be run in non-interactive mode by supplying the parameters that the interactive mode asks for. Read the section below on **api-sync create** to understand the exact syntax for supplying those parameters. The usage is as follows:
...
The create option is perfect for taking a newly designed API and uploading it to the Anypoint platform. Remember that api-synch should always been run from the working directory where the root RAML file is kept. Here are some example usages:

This usage would be for uploading a brand new API to Anypoint where your name for the new API is "Acme Customer API" and your version number is "1" (trailing zeroes are truncated so specifying "1.0" is not necessary):
...
- The version of the new API is "1"
...
If you wanted the name of the API to be "Acme Customer", then the usage would be as follows (note the quotation marks):

```
api-sync create --bizGroup=12345 --apiName="Acme Customer" --apiVersion=1 --rootRaml=acme.raml
```

**Example usage when adding a new version to an API that already exists on the Anypoint platform**
In this case, you will need the ID of the existing API
